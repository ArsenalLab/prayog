...
// Strict output validation schema for Agent 1 (Error Diagnostics)
export const ErrorProfileSchema = z.object({
    status: z.enum(['error', 'success']),
    errorCode: z.string().nullable().describe("Standard DB2 error code (e.g. SQL0102N) or system error code"),
    errorMessage: z.string().describe("The extracted main error message text"),
    stackTrace: z.string().nullable().describe("Extracted stack trace lines leading to the failure"),
    failureContext: z.string().describe("A brief summary of the conditions or sequence of events leading to the failure"),
    timestamp: z.string().describe("ISO timestamp or raw log timestamp when the error occurred")
});
...
1. Locate and read the run logs in the directory or file specified below:
   Log file path to inspect: "{{LOG_FILE_PATH}}"
...
- Locate the log file at the path: "{{LOG_FILE_PATH}}". Use your tools to read it.
